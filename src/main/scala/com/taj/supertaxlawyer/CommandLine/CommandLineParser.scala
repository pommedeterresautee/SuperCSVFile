...
class CommandLineParser(args: Array[String]) extends ScallopConf(args) {
  ...
  val outputFile = opt[String]("outputFile", descr = "Path to the file where to save the results.", validate = path ⇒ !new File(path).isDirectory && new File(path).getParentFile.isDirectory)
